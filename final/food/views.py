...
def donar_donation_view(request):
    if request.method == "POST":
        ...
        message = f"Hello, Charity Application!\n\nA new food donation has been made.\nDonor: {don.username}\nFood Name: {don.foodName}\nQuantity: {don.quantity}\nAddress: {don.address}\n\nThank you!"
        from_email = settings.EMAIL_HOST_USER  # Replace with your email or use settings.DEFAULT_FROM_EMAIL
        recipient_list = ['akaviyameena@gmail.com']  # Replace with the email address of the charity application
...
def frequently_donated_donors_view(request):
    frequently_donated_donors = Donation.objects.values('username').annotate(
        total_quantity=Sum('quantity'),
        total_number=Sum('number')
    ).order_by('-total_quantity', '-total_number')[:5]

    return render(request, 'frequently_donated_donors.html', {'frequently_donated_donors': frequently_donated_donors})
